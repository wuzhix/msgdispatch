...
use Juanpiorg\Wechatcorp\WeChatCorpMessageApi;
...
class WechatWorker
{
    ...
    public function start()
    {
        ...
        $this->initMongo();
        $redis_key = $this->getConf('wechat_redis_key');
        while (true) {
     ...
     * @return bool
     */
    protected function dealWechat($wechat_info)
    {
        //echo $wechat_info."\n";
        $data = json_decode($wechat_info, true);
        if (empty($data)) {
            return false;
        }
        ...
        $corp_id = '';
        $secret = '';
        $wechat = new WeChatCorpMessageApi($corp_id, $secret);
        $result = $wechat->sendText($data['appid'], $data['to'], $data['body'], $data['type'], $data['is_private']);
        if ($result) {
            ...
            $db_collection = $this->getConf('db_wechat_collection');
            ...
            if (!isset($data['send_reply'])) {
                $data['send_reply'] = 1;
                $this->redis->rPush($this->getConf('wechat_redis_key'), json_encode($data));
            } else {
