...
class EmailWorker
{
    ...
    protected function initSql()
    {
        if ($this->sql_connect == null) {
            $this->sql_connect = mysqli_connect(
                $this->getConf('db_ip'),
                $this->getConf('db_username'),
                $this->getConf('db_password')
            );
            mysqli_select_db($this->sql_connect, $this->getConf('db'));
            mysqli_query($this->sql_connect, "set character set 'utf8'");//读库
            mysqli_query($this->sql_connect, "set names 'utf8'");//写库
        } else {
            if (!mysqli_ping($this->sql_connect)) {
                ...
                mysqli_select_db($this->sql_connect, $this->getConf('db'));
                mysqli_query($this->getConf('db'), "set character set 'utf8'");//读库
                mysqli_query($this->getConf('db'), "set names 'utf8'");//写库
            }
        }
    }
    ...
    protected function recordData($table, $remark, $title, $data)
    {
        ...
        //echo $create_sql."\n";
        mysqli_query($this->sql_connect, $create_sql);
        ...
        //echo $insert_sql."\n";
        mysqli_query($this->sql_connect, $insert_sql);
    }
}
